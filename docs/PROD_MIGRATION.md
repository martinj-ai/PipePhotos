...
- **Patchright** (Playwright undetected fork) + **Chrome système** (channel='chrome') pour bypass DataDome/Akamai sur Expedia + Marriott/Hilton/Hyatt/IHG
...
| **Patchright** | `patchright install chromium` dans le build step. ⚠️ **Chrome système** : Railway containers Linux n'ont pas Chrome installé par défaut — il faut soit installer `google-chrome-stable` via apt, soit fallback sur le chromium bundled (moins efficace contre DataDome) |
| **`headless=False`** | ⚠️ **Impossible en serveur headless Railway** sans display Xvfb. Options : (1) Xvfb + `headless=False` (overhead ~50MB RAM + complexité), (2) accepter de tomber sur le chromium headless (probable 50% de taux de bypass), (3) ScraperAPI/Bright Data en alternative payante ($50-200/mo selon volume) |
...
| Workflow Booking-first : URL Booking = clé d'entrée unique + auto-discovery URLs annexes | `data/rp/booking-<slug>.json` | `pipeline_runs.input_url` (booking) + `hotels.urls` JSONB { official, expedia, rp } | `GEMINI_API_KEY` | async ThreadPool 3 | **Refactor UX (Martin 12/05/2026)** : avant, l'utilisateur saisissait séparément l'URL RP OU l'URL Booking. Maintenant : Étape 1 = URL Booking obligatoire → backend appelle `/api/identify-hotel` qui (a) scrape Booking (nom + ville + amenities), (b) lance en parallèle 3 finders (`hotel_site_finder`, `expedia_finder`, `rp_finder`) avec cascade DDG primary + Gemini fallback. Latence totale ~5-8s. Étape 2 affiche les 4 URLs (Booking + 3 trouvées) avec badge source (DDG vert / Gemini violet) — toutes éditables avant lancement. Cohérence en prod : `hotels.urls` JSONB + cache des résultats de finder pour éviter re-DDG sur replay. |
| Cascade DDG primary + Gemini fallback pour 3 finders (hotel_site_finder, expedia_finder, rp_finder) | — | — | `GEMINI_API_KEY` (fallback only) | sync (3-8s) | **Pourquoi DDG en primary** : DDG HTML free, sans API key, retourne les vraies URLs canoniques indexées (vs Gemini qui peut halluciner — observé sur Expedia ID `h67900000`). Pattern : DDG query → regex match URL canonique → si rate-limit ou 0 hit → retry +3s → fallback Gemini avec prompt JSON strict. Détecteur d'hallucination sur Expedia : `_looks_hallucinated()` rejette les IDs avec 4+ zéros consécutifs ou chiffres tous identiques. En prod : observer le ratio DDG_hit / Gemini_fallback (devrait être ~80/20) pour détecter une rate-limit cumulative côté IP Railway. |
| Bypass Expedia DataDome via patchright + Chrome réel + headless=False | — | — | — | sync (~30s) | **Découverte empirique (Martin 12/05/2026)** : Playwright stealth + Chromium bundled + headless=True → page « Bot or Not? » systématique (DataDome). Patchright (fork undetected) + `channel='chrome'` (Chrome système macOS) + `headless=False` (DataDome détecte mode headless via JS fingerprint) = bypass à 100%. Conséquence locale : fenêtre Chrome visible ~30s pendant le scrape Expedia. ⚠️ **Pas portable en prod Railway sans Xvfb** : un container Linux headless n'a pas de display. Options : (1) Xvfb wrapping (overhead ~50MB), (2) accepter le fallback chromium headless (probable 50% bypass), (3) ScraperAPI ($50/mo). Module : `expedia_scraper.py`, exception `ExpediaBotBlocked` levée si bot wall détectée même avec patchright. |
| Bypass Akamai/Imperva chaînes hôtelières (Marriott, Hilton, Hyatt, IHG) via cascade auto | — | — | — | sync (10-30s selon cascade) | **Découverte empirique (Martin 12/05/2026)** : Marriott sert « Access Denied » (HTML 342 bytes, 0 imgs) au Playwright stealth headless. Patchright + Chrome réel + headless=False bypass → vraie page 824KB, 35 imgs cache.marriott.com. **Cascade automatique** dans `hotel_gallery_extractor.py` : tentative A = stealth headless (rapide ~10s, invisible), détection signaux blocage (title "Access Denied"/"Just a moment", HTML < 5KB, ou 0 photo après crawl), tentative B = patchright + Chrome + headless=False si bloqué. **Garde le résultat avec le plus de photos**. Yotel/Drupal non-bloqués passent par tentative A seulement (pas de Chrome visible). En prod : besoin de Xvfb sur Railway (cf. note Expedia ci-dessus). |
| Sources HD : Booking 3000×2000 (path max3000) + Expedia 3840×2560 (URL nue) | — | `photos.width` + `photos.height` colonnes | — | sync OK | **Découverte empirique (Martin 12/05/2026)** : (1) Booking CDN `cf.bstatic.com` : la signature HMAC `k=` est universelle — elle marche pour TOUTES les tailles du path (`max300`, `max500`, ..., `max3000`). On remplace donc `max1024x768` → `max3000` avant retour de `scrape_booking_photos()` → 3000×2000 natif. (2) Expedia CDN `images.trvl-media.com` : l'URL NUE (sans `?impolicy=resizecrop&rw=1200`) sert l'image originale en **3840×2560** (4K natif). On strip donc tous les query params. Gain x3 résolution et x5-6 poids fichier mais bénéfice pHash + PDF avant/après + crops finaux. Modules : `booking_scraper.py` (constante `BOOKING_TARGET_SIZE = "max3000"`), `expedia_scraper.py` (`_strip_to_original` dans la dédup). |
| Upscale Lanczos x2 systématique sur toutes les photos finales | `data/output/<slug>/enhanced/` (overwrite in-place) | — | — | sync (~0.3s/photo) | **Feature qualité (Martin 12/05/2026)** : Nano Banana 2 sort à 1264×843 fixe (testé empiriquement, indépendant de la source). Les UIs Dayuse desktop sur Retina/4K affichent ces photos en 1500-1800px CSS = effectivement 3000-3600px physiques → pixelisation visible. Solution : `upscale_lanczos_inplace()` dans `enhance.py` appelée APRÈS la LUT brand sur le `output_path` final. Lanczos = interpolation classique (PIL `Image.Resampling.LANCZOS`), n'ajoute pas de détail réel mais évite la pixelisation et préserve les arêtes. Coût zéro, désactivable via env `FINAL_UPSCALE_FACTOR=1.0`. Photo finale : 2528×1686 (vs 1264×843). Le nœud `upscale_lanczos` est trackable dans `photo_journey` pour la viz workflow. Real-ESRGAN (vraie super-résolution IA, Replicate ~$0.001/img) reste en backlog si Lanczos jugé insuffisant un jour. |
| Instagram retiré comme source | — | (anciennes colonnes `instagram_*` à drop si déjà créées) | — | — | **Décision Martin (12/05/2026)** : Instagram bloque systématiquement le scraping anonyme depuis 2024 ("Profile isn't available" sur tous les bots) → la source ramenait 0 photo dans 100% des cas. Imports `instagram_finder`/`instagram_scraper` retirés de `app.py`. Endpoint `/api/identify-hotel` passe de 4 workers parallèles à 3 (rp + expedia + official). Sources_summary, priority dédup, SOURCE_BADGE, SOURCE_LABELS, checkbox UI tous nettoyés. Les fichiers `instagram_finder.py` et `instagram_scraper.py` restent sur disque pour réutilisation si une API tierce arrive (RapidAPI Instagram Scraper ~$10-30/mo en backlog). |
| PDF export avant/après branded Dayuse | `data/output/<slug>/pdf/{slug}_pack.pdf` | nouvelle table `pdf_exports` (run_id, cdn_url, created_at) | — | sync (~5-10s) | **Feature Martin** : génère un PDF Dayuse-branded (Manrope + gradient golden #FFAF36→#FFC536) avec : (a) page cover + 4 stat cards (photos sourcées, finales, durée pipeline, coût IA), (b) preview Dayuse.fr mockup (hero grid 1 large + 2 small + reviews + map), (c) 1 page par photo finale avec avant/après side-by-side + chips transformations. Implémenté via **Playwright `page.pdf()`** (PAS WeasyPrint — évite dep système). Images en base64 data URLs pour portabilité. Signature discrète "Martin Juglair" en footer. Route : `POST /api/export/<slug>.pdf`. Template Jinja2 : `templates/pdf_export.html`. En prod : stocker le PDF généré sur S3/R2 avec URL signée 7j. |
| LUT brand adaptative (profils soft/medium/strong selon ambiance) | `config/brand_lut.json` | param table | — | sync OK | **Itération (Martin 12/05/2026)** : v1 LUT unique → photos sources lumineuses+chaudes ressortaient "trop jaune". v2 adaptative : `brand_lut.pick_profile(analysis)` choisit selon ambiance Gemini + palette : (a) lumineux-chaud + aligned-warm → profile `soft` (sat 1.12, warmth_r 1.03), (b) standard → `medium` (sat 1.15, warmth_r 1.05), (c) lumineux-froid / off-brand → `strong` (sat 1.20, warmth_r 1.10). Profile injecté dans la strategy via `lut_profile`, propagé à `enhance_local_warm` et `enhance_one`. En prod : table `brand_lut_versions` permettant rollback + AB test. |
| SEO filename mapping (dayuse_{slug}_{amenity}_{seq:02d}.jpg) | `data/uploads/<slug>/seo_names.json` | table `photos.seo_filename` | — | sync OK | **Feature UX Martin** : nom fichier d'origine `booking_001_461638112.jpg` peu lisible côté éditorial → mapping persistant vers `dayuse_aloft-miami-airport_pool_01.jpg` (slug-hotel + amenity EN + séquence). Amenity = catégorie Gemini Vision normalisée (pool/cabana/spa/rooftop/beach/gym/lobby/restaurant/bar/view/aerial). Stocké dans `seo_names.json` pour consistency UI + ZIP + PDF. Helpers `_seo_slug_from_name()`, `_seo_amenity_for_file()`, `_seo_filename()`, `_persist_seo_names_map()`. En prod : DB colonne `photos.seo_filename` (UNIQUE per hotel_slug). |
| Workflow viz : nœud `upscale_lanczos` ajouté entre `lut_brand` et `final_pack` | — | — | — | sync OK | **Doc front (Martin 12/05/2026)** : `photo_journey.py` NODE_DEFINITIONS passe de 15 à 16 entrées. Le nœud est trackable par photo dans le canvas workflow + visible dans le diagramme Mermaid de la sous-onglet Documentation. En prod : aucune action requise, juste un champ JSON enrichi dans `pipeline_runs.workflow_nodes`. |
